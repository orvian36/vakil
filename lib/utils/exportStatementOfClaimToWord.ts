...
function getOrdinalSuffix(num: number): string {
  ...
  function parseMarkdownToFormattedParagraphs(markdown: string, customLineSpacing: number = 360): (Paragraph | Table)[] {
    ...
    let numberingInstance = 0; // Track numbering instance for continuous lists
    let currentNestingLevel = 0; // Track current nesting level: 0 = top (1.), 1 = (a), 2 = (i)
    let inList = false;
    let lastTopLevelInstance = -1; // Track the instance used for the last top-level item
    
    for (let i = 0; i < lines.length; i++) {
      const line = lines[i].trim();
      
      // Skip empty lines but use them to break paragraphs and lists
      if (!line) {
        if (currentParagraphLines.length > 0) {
          paragraphs.push(createFormattedParagraphFromLines(currentParagraphLines, customLineSpacing));
          currentParagraphLines = [];
        }
        // Empty line breaks list continuity
        // But check if next line continues the list before breaking
        if (inList) {
          ...
          if (nextLineIndex < lines.length) {
            ...
            const isNextNumbered = nextLine.match(/^\*\*(\d+\.)\*\*\s*/) || nextLine.match(/^(\d+\.\s+)/);
            const isNextSubItem = nextLine.match(/^\*\*\(([a-z]|i{1,3}|iv|v|vi{0,3}|xi{0,2}|x)\)\*\*\s*/i) || 
                                   nextLine.match(/^(\(([a-z]|i{1,3}|iv|v|vi{0,3}|xi{0,2}|x)\)\s+)/i);
            if (!isNextNumbered && !isNextSubItem) {
              // Next line is not a list item, break the list
              inList = false;
              lastTopLevelInstance = -1;
              numberingInstance++;
            }
            // If next line is a list item, keep same instance
          } else {
            // No more lines, break the list
            inList = false;
            lastTopLevelInstance = -1;
            numberingInstance++;
          }
      ...
      if (line.startsWith('#')) {
        ...
        // Headings break list continuity
        if (inList) {
          inList = false;
          numberingInstance++;
        }
      ...
      // Check for numbered paragraphs (main level: 1., 2., etc.) - Level 0
      // Handle both "1. " and "**1.** " formats (with optional space after bold markers)
      ...
      if (numberedMatch) {
        ...
        // If we have a previous top-level item in the same list, use its instance
        // Otherwise, start a new instance
        if (lastTopLevelInstance >= 0 && inList) {
          // Continue the same top-level sequence
          numberingInstance = lastTopLevelInstance;
        } else {
          // Start a new top-level sequence
          if (!inList) {
            numberingInstance++;
          }
          lastTopLevelInstance = numberingInstance;
        }
        ...
        paragraphs.push(createNumberedParagraph(line, customLineSpacing, numberingInstance, 0));
        continue;
      }
      
      // Check for roman numeral sub-items (i), (ii), (iii), etc. - Level 2
      // MUST check before letters to avoid matching (i) as a letter
      // Handle both "(i) " and "**(i)** " formats (with optional space after bold markers)
      const romanMatch = line.match(/^\*\*\((i{1,3}|iv|v|vi{0,3}|xi{0,2}|x)\)\*\*\s*(.*)$/i) || line.match(/^(\((i{1,3}|iv|v|vi{0,3}|xi{0,2}|x)\)\s+)(.*)$/i);
      if (romanMatch) {
        // Finish current paragraph if any
        if (currentParagraphLines.length > 0) {
          paragraphs.push(createFormattedParagraphFromLines(currentParagraphLines, customLineSpacing));
        ...
        // Set to level 2 (roman numerals)
        currentNestingLevel = 2;
        
        // Continue current list or start new one
        if (!inList) {
          inList = true;
        }
        
        paragraphs.push(createSubNumberedParagraph(line, customLineSpacing, numberingInstance, 2));
        continue;
      }
      
      // Check for sub-numbered items (a), (b), (c), etc. - Level 1
      // Exclude 'i' from letter match since it's handled as roman numeral above
      // Handle both "(a) " and "**(a)** " formats (with optional space after bold markers)
      const subNumberedMatch = line.match(/^\*\*\(([a-hj-z])\)\*\*\s*(.*)$/) || line.match(/^(\([a-hj-z]\)\s+)(.*)$/);
      if (subNumberedMatch) {
        // Finish current paragraph if any
        if (currentParagraphLines.length > 0) {
          paragraphs.push(createFormattedParagraphFromLines(currentParagraphLines, customLineSpacing));
        ...
        paragraphs.push(createSubNumberedParagraph(line, customLineSpacing, numberingInstance, 1));
        continue;
      }
      ...
      // Regular text line - accumulate for paragraph and break list continuity
      if (inList) {
        inList = false;
        lastTopLevelInstance = -1; // Reset top-level tracking
        numberingInstance++;
      }
      currentParagraphLines.push(line);
    }
  ...
  // Function to create a numbered paragraph (1., 2., etc.) - Level 0
  // Handles both "1. text" and "**1.** text" formats
  function createNumberedParagraph(line: string, lineSpacing: number, numberingInstance: number, level: number = 0): Paragraph {
    // Try to match "**1.** text" format first (with optional space)
    let match = line.match(/^\*\*(\d+\.)\*\*\s*(.*)$/);
    if (!match) {
      // Fall back to "1. text" format
    ...
    const text = match[2] || ''; // Extract text without the number, default to empty string
  ...
  // Function to create a sub-numbered paragraph ((a), (b), (i), (ii), etc.)
  // Handles both "(a) text" and "**(a)** text" formats
  // level: 1 for letters (a), 2 for roman numerals (i)
  function createSubNumberedParagraph(line: string, lineSpacing: number, numberingInstance: number, level: number): Paragraph {
    // Try to match "**(a)** text" or "**(i)** text" format first (with optional space)
    let match = line.match(/^\*\*\(([a-z]|i{1,3}|iv|v|vi{0,3}|xi{0,2}|x)\)\*\*\s*(.*)$/i);
    let text: string;
    let numeralOrLetter: string = '';
    
    if (match) {
      // Bold format: match[1] = letter/numeral, match[2] = text
      numeralOrLetter = match[1];
      text = match[2] || '';
    } else {
      // Fall back to "(a) text" or "(i) text" format
      match = line.match(/^(\(([a-z]|i{1,3}|iv|v|vi{0,3}|xi{0,2}|x)\)\s+)(.*)$/i);
      if (!match) {
        return createFormattedParagraphFromLines([line], lineSpacing);
      }
      // Plain format: match[1] = full "(a) ", match[2] = letter/numeral, match[3] = text
      numeralOrLetter = match[2];
      text = match[3] || '';
    }
    
    // For level 2 (roman numerals), display the exact parsed value instead of using Word's numbering
    if (level === 2) {
      // Create a paragraph with the exact roman numeral as part of the text
      const romanNumeral = `(${numeralOrLetter}) `;
      const fullText = romanNumeral + text;
      const formatInfos = parseMarkdownFormatting(fullText);
      const textRuns = createTextRunsFromFormatInfo(formatInfos, 24);
      ...
      return new Paragraph({
        ...
        alignment: AlignmentType.JUSTIFIED,
        indent: {
          left: 1440, // Same indentation as level 2 numbering
          hanging: 360
        },
        children: textRuns
      });
    ...
    // For level 1 (letters), use Word's numbering system as before
    const formatInfos = parseMarkdownFormatting(text);
    const textRuns = createTextRunsFromFormatInfo(formatInfos, 24);
...
function getRoleLabel(party: any, plaintiffs: any[], defendants: any[],index: number): (TextRun | Math_2)[] {
  ...
  // Function to generate the Statement of Claim document
  function generateStatementOfClaim(data: LegalDocumentData, caseData: Case) {
      ...
      sections: [{
        ...
        children: [
          ...
          new Paragraph({
            children: [
              new TextRun({
                text: "I believe that the facts stated in this Statement of Claim are true.",
                font: "Times New Roman",
                size: 24,
                color: "000000"
              })
          ...
          new Paragraph({
            children: [
              new TextRun({
                text: "æœ¬äººç›¸ä¿¡æœ¬ç”³ç´¢é™³è¿°æ›¸æ‰€è¿°äº‹å¯¦å±¬å¯¦ã€‚",
                font: "Noto Sans SC",
                size: 24,
                color: "000000"
              })
  ...
  // Function to extract Statement of Claim content between markers
function extractSOCContent(text: string): string {
    const socMarker = '# Statement of Claim\n';
    //const datedMarker = '\nDated';
...
export async function downloadStatementOfClaimAsWord(options: LegalDocumentData): Promise<void> {
    //const data = createLegalDocumentData(options);
  try {
    const blob = await generateStatementOfClaim(options,options.caseData);
    ...
    console.error('Error exporting StatementOfClaim to Word:', error);
    throw new Error('Failed to export StatementOfClaim document to Word format');
  }
}
