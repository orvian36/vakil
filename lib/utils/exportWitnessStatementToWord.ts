...
function getOrdinalSuffix(num: number): string {
  ...
  function parseMarkdownToFormattedParagraphs(markdown: string, customLineSpacing: number = 360): (Paragraph | Table)[] {
    ...
    let subNumberingInstance = 0; // Track sub-numbering instance
    let inList = false;
    
    for (let i = 0; i < lines.length; i++) {
      const line = lines[i].trim();
      
      // Skip empty lines but use them to break paragraphs (but NOT lists)
      if (!line) {
        if (currentParagraphLines.length > 0) {
          paragraphs.push(createFormattedParagraphFromLines(currentParagraphLines, customLineSpacing));
          currentParagraphLines = [];
        }
        // Empty lines do NOT break list continuity - lists continue across empty lines
        continue;
      }
      ...
      if (line.match(/^\d+\.\s+/)) {
        ...
        // Each numbered item starts a new sub-numbering instance
        subNumberingInstance++;
        
        paragraphs.push(createNumberedParagraph(line, customLineSpacing, numberingInstance));
        continue;
      }
      
      // Check for sub-numbered items (1), (2), (3), etc. - CHANGED FROM (a), (b), (c)
      if (line.match(/^\(\d+\)\s+/)) {
        // Finish current paragraph if any
        if (currentParagraphLines.length > 0) {
          paragraphs.push(createFormattedParagraphFromLines(currentParagraphLines, customLineSpacing));
        ...
        // Check if this is "(1)" - if so, start a new sub-list instance
        if (line.match(/^\(1\)\s+/)) {
          subNumberingInstance++;
        }
        paragraphs.push(createSubNumberedParagraph(line, customLineSpacing, subNumberingInstance));
        continue;
      }
  ...
  function createNumberedParagraph(line: string, lineSpacing: number, numberingInstance: number): Paragraph {
    const match = line.match(/^(\d+\.\s+)(.*)$/);
    if (!match) {
      return createFormattedParagraphFromLines([line], lineSpacing);
    }
    
    const text = match[2]; // Extract text without the number
  ...
  // Function to create a sub-numbered paragraph ((1), (2), etc.) - CHANGED FROM (a), (b)
  function createSubNumberedParagraph(line: string, lineSpacing: number, numberingInstance: number): Paragraph {
    const match = line.match(/^(\(\d+\)\s+)(.*)$/);
    if (!match) {
      return createFormattedParagraphFromLines([line], lineSpacing);
    }
    
    const text = match[2]; // Extract text without the sub-number
    const formatInfos = parseMarkdownFormatting(text);
    const textRuns = createTextRunsFromFormatInfo(formatInfos, 24);
    
    // Use separate numbering reference for each sub-list group
    const subListReference = `sub-list-${Math.min(numberingInstance, 5)}`; // Cap at 5 for now
    
    return new Paragraph({
      numbering: {
        reference: subListReference,
        level: 0,
        instance: 0 // Always use instance 0 since each reference is separate
      },
      children: textRuns
    });
  }
...
function getRoleLabel(party: any, plaintiffs: any[], defendants: any[],index: number): (TextRun | Math_2)[] {
  ...
  // Function to generate the Witness Statements document
  function generateWitnessStatements(data: LegalDocumentData, caseData: Case) {
    ...
    const doc = new Document({
      ...
      numbering: {
        config: [
          ...
          // Create multiple sub-list configurations with numeric format
          {
            reference: "sub-list-1",
            levels: [
              {
                level: 0,
                format: LevelFormat.DECIMAL,
      ...
      sections: [{
        ...
        children: [
          ...
          new Paragraph({
            ...
            children: [
              new TextRun({
                text: "WITNESS STATEMENT OF " + data.plaintiffName,
                bold: true,
                font: "Times New Roman",
                size: 24
              })
          ...
          new Paragraph({
            children: [
              new TextRun({
                text: "I believe that the facts stated in this witness statement are true, and (if applicable) the opinions expressed therein are genuinely held.",
                font: "Times New Roman",
                size: 24,
                color: "000000"
              })
...
function extractSOCContent(text: string): string {
    ...
    const bengaliMarker = '# à¦¸à¦¾à¦•à§à¦·à§€à¦° à¦¬à¦¿à¦¬à§ƒà¦¤à¦¿\n';
    ...
    const afterSOC = text.substring(startIndex + markerLength);
...
export async function downloadWitnessAsWord(options: LegalDocumentData): Promise<void> {
    //const data = createLegalDocumentData(options);
  try {
    const blob = await generateWitnessStatements(options,options.caseData);
    ...
    console.error('Error exporting Summary to Word:', error);
    throw new Error('Failed to export Summary document to Word format');
  }
}