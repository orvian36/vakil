...
import { FetchFileDataAgent, GenerateChronologyAgent } from '@/lib/orchestration/agents';
...
const loadConfig = (): AgentConfig[] => {
  try {
    const configPath = path.join(process.cwd(), 'lib', 'orchestration', 'chronology_generate_config.js');
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("caseId from generate/chronology", caseId);
    ...
    // Extract the chronology content and thinking from the result
    const chronology = result.chronology;
    const content = chronology?.content || '';
    const thinking = chronology?.thinking || '';

    console.log('🎉 Chronology generation completed successfully!');
